...
def train_step(model: torch.nn.Module, 
    ...
    for batch, (pcp, ms, nl, y) in enumerate(tqdm(dataloader)):    
            
        # Send data to target device
        pcp, ms, nl, y =  pcp.float(), ms.float(), nl.float(), y.float()
        pcp, ms, nl, y = pcp.to(device), ms.to(device), nl.to(device), y.to(device)

        # 1. Forward pass
        y_pred = model( pcp, ms, nl )
