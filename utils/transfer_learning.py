...
def update_single_layer(model):
    ...
    new_layer = torch.nn.Conv2d(in_channels=1, out_channels=64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3),
                                bias=False)
    with torch.no_grad():
...
def update_first_layer(model, in_channels, weights_init='random', scaling=1.):
    """changes the first Conv2d layer to take in inputs as many _in_channels_
    ...
    new_layer = torch.nn.Conv2d(in_channels=in_channels, out_channels=64, kernel_size=(7, 7), stride=(2, 2),
                                padding=(3, 3), bias=False)
    # For now, all weights of added input channels are set to 0. 
    # swapping red - blue channels.
...
def s2_to_landsat(model: torch.nn.Module, scaling: float) -> torch.nn.Module:
    ...
    """
    conv1 = model.conv1
    new_layer = torch.nn.Conv2d(in_channels=7, out_channels=64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3),
                                bias=False)
