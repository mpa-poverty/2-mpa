...
def compute_average_crossval_results(results: dict):
    result_list = []
    for fold in results:
        fold_result = [np.array(results[fold]['test_r2'][i].cpu().numpy())[()] for i in
                       range(len(results[fold]['test_r2']))]
...
def standardize_countryname(countryname: str) -> str:
    ...
    if countryname in ['democratic_republic_of_congo', 'Democratic Republic of the Congo',
...
def build_series_from_dict(series_dict, row, series_length, num_series, num_years, normalizer, variable_name,
    ...
    for year in range(row.year - num_years + 1, row.year + 1):
        ...
        if unit == 'month':
            ...
            monthly_series[0 * (start_index + 1):12 * (start_index + 1)] = np.array(
                series_dict[(row.country, row.year, year, int(row.cluster))])
        else:
